# Fully orchestrated planning

Create one coherent plan for a complex request that genuinely spans independent domains or workstreams.
...
Use workstream ownership, dependency order, integration gates, cross-boundary risks, verification responsibilities, architecture, tests, implementation, migration, security, rollout, rollback, and repository evidence only where the request makes them relevant. Reconcile conflicting information and return one resolved plan rather than a bundle of suggestions; do not invent multiple workstreams or repositories.
