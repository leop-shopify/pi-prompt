...
if (!capability) byId("auth-lost").hidden = false;
  ...
  let planSequence = 0; let specSequence = 0; let stopped = false; let progressFrame = 0; let composerState = null; const enteredPlanRebuild = createRebuildTransitionTracker();
  ...
  const applyPlanSnapshot = (snapshot) => {
    ...
    const enteredRebuild = enteredPlanRebuild(next);
    api.setVersion(next.stateVersion); store.set({ snapshot: next, clarificationDraft, ...(sameRevision ? {} : { selectedAnnotationIds: [], selectedGrillAnnotationIds: [] }) });
    if (enteredRebuild) window.scrollTo({ top: 0, behavior: "auto" });
    if (!next.grill && current.activeStage !== "plan" || !sameRevision) closeComposer(false);
  };
  const applySpecSnapshot = (snapshot) => { if (!snapshot) { store.set({ specSnapshot: null }); return; } api.setSpecVersion(snapshot.stateVersion); store.set({ specSnapshot: snapshot }); };
  const refreshPlan = async () => { const snapshot = await api.snapshot(); const markdown = await api.plan(); applyPlanSnapshot(markdown !== null ? { ...snapshot, planMarkdown: markdown } : snapshot); };
  const refreshSpec = async () => { const snapshot = await api.specSnapshot(); if (!snapshot) { applySpecSnapshot(null); return null; } const markdown = await api.specMarkdown(); const next = markdown === null ? snapshot : { ...snapshot, markdown }; applySpecSnapshot(next); return next; };
  const mutatePlan = async (path, method, body) => { const result = await api.mutate(path, method, { requestId: requestId(), ...body }); if (result?.snapshot) applyPlanSnapshot(result.snapshot); return result; };
  const mutateSpec = async (path, method, body) => { const result = await api.specMutate(path, method, { requestId: requestId(), ...body }); if (result?.snapshot) applySpecSnapshot(result.snapshot); return result; };
  ...
  const runAction = async (operation) => {
    ...
    try { await operation(); }
      ...
      if (error?.snapshot) error.kind === "spec" ? applySpecSnapshot(error.snapshot) : applyPlanSnapshot(error.snapshot);
  ...
  const openItem = (stage, item, anchor) => {
    ...
    byId("selection-provenance").textContent = generated ? "Grill critique" : "Your comment"; byId("selection-label").textContent = generated ? `Generated critique (${item.status})` : `Edit ${stage === "spec" ? "Spec comment" : "Plan comment"} (${item.status})`; byId("selection-comment").value = item.body;
  ...
  const draw = (state) => {
    ...
    const rebuilding = isPlanRebuilding(snapshot); const planSurface = byId("plan-surface"); const rebuildOverlay = byId("plan-rebuild-overlay"); planSurface.setAttribute("aria-busy", String(rebuilding)); rebuildOverlay.hidden = !rebuilding || !planView;
    ...
    const labels = activeStage === "plan" ? ["Durable original", "Plan · Review and annotate", "Your selected-text comments stay attached to this durable Plan revision."] : activeStage === "grill" ? ["Critique overlay", "Grill · Same Plan, challenged", "Generated critiques are immutable overlays on the same Plan. Your comments remain editable."] : ["Independent artifact", "To Spec · Implementation specification", "This separately versioned Markdown Spec has its own comments, revisions, and acceptance."];
    ...
    const errorValue = activeStage === "spec" ? specSnapshot?.error : snapshot.error; const stale = activeStage === "spec" && specIsStale(snapshot, specSnapshot); const error = byId("snapshot-error"); error.hidden = !errorValue && !stale; error.textContent = stale ? "This Spec source is stale because its Plan or Grill changed. Generate a fresh Spec from the current Plan and Grill." : errorValue ? `${errorValue.message} (${errorValue.code})` : "";
    ...
    const runGrill = byId("run-grill-button"); const grillRunnable = canRunGrill(snapshot, specSnapshot); runGrill.hidden = activeStage !== "plan" || !grillRunnable; runGrill.disabled = busy || !grillRunnable; runGrill.textContent = snapshot.grill || snapshot.status === "error" ? "Retry Grill" : "Run Grill";
  ...
  function drawProgress(snapshot, specSnapshot, stage) {
    ...
    byId("progress-eyebrow").textContent = spec ? "To Spec" : job?.operation === "grill" ? "Grilling" : "Thinking";
    const failure = spec ? !job && specSnapshot?.error : !job && snapshot.error; byId("progress-headline").textContent = failure ? `${spec ? "Spec" : job?.operation === "grill" ? "Grill" : "Plan"} generation needs attention` : spec ? job?.operation === "revision" ? "Revising the Spec" : "Generating the Spec" : job?.operation === "grill" ? "Grilling the current Plan" : activity?.headline ?? "Planning is starting";
    byId("progress-detail").textContent = failure ? failure.message : spec ? "Building the independent implementation Spec from the exact Plan and Grill source." : activity?.progress?.summary ?? activity?.summary ?? (job?.operation === "grill" ? "Generating immutable critiques without rewriting the Plan." : "Preparing the planning run.");
  ...
  byId("run-grill-button").addEventListener("click", () => { void runAction(async () => { await mutatePlan("/api/v1/grill-runs", "POST", {}); store.set({ activeStage: "grill" }); toast("Grill started against this exact Plan revision."); }); });
  ...
  byId("spec-generate-button").addEventListener("click", () => { void runAction(async () => { const current = store.get(); const fresh = canGenerateFreshSpec(current.snapshot, current.specSnapshot); await mutateSpec(fresh ? "/api/v1/spec/fresh-generations" : "/api/v1/spec/generations", "POST", {}); if (fresh) store.set({ selectedSpecCommentIds: [] }); toast(fresh ? "Fresh Spec generation started from the current Plan and Grill." : "Spec generation started."); }); });
  ...
  const stopSpec = (disposition) => { void runAction(async () => { if (!(await confirmDialog(disposition === "pause" ? "Pause Spec work?" : "Cancel this Spec?", disposition === "pause" ? "The independent Spec history remains saved." : "The Spec becomes terminal; the Plan and Grill remain unchanged.", disposition === "pause" ? "Pause Spec" : "Cancel Spec"))) return; await mutateSpec("/api/v1/spec/cancel", "POST", { disposition }); }); };
