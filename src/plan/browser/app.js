...
import {
  ...
  canAccept, canRetryStaging, canSubmitPlan,
} from "./store.js";
...
if (!capability) byId("auth-lost").hidden = false;
  ...
  const draw = (state) => {
    ...
    for (const stage of ["plan", "grill", "spec"]) {
      ...
      const reviewSkipped = stage === "grill" && specSnapshot?.source && specSnapshot.source.grillBasedOnDocumentRevision === undefined;
      byId(`stage-${stage}-state`).textContent = current ? working ? "Current · busy" : "Current" : working ? "Busy" : reviewSkipped ? "Skipped" : available ? stage === "plan" ? "Available" : stage === "grill" && snapshot.grill ? "Complete" : stage === "spec" && specSnapshot?.markdown ? "Generated" : "Optional" : stage === "grill" && availability.spec ? "Optional" : "Unavailable";
    }
    ...
    const errorValue = activeStage === "spec" ? specSnapshot?.error : snapshot.error; const stale = activeStage === "spec" && specIsStale(snapshot, specSnapshot); const error = byId("snapshot-error"); error.hidden = !errorValue && !stale; error.textContent = stale ? "This Spec source is stale because its Plan or optional Adversarial Review changed. Generate a fresh Spec from the current source." : errorValue ? `${errorValue.message} (${errorValue.code})` : "";
    ...
    const reviewPlanFeedback = byId("review-plan-with-feedback-button"); reviewPlanFeedback.hidden = activeStage !== "grill" || selectedGrill.length === 0; reviewPlanFeedback.disabled = grillBusy || !canAddressGrillFeedback(snapshot, state.selectedGrillAnnotationIds);
    const retryPlanStage = canRetryStaging(snapshot); const planSubmissionAvailable = canAccept(snapshot) || retryPlanStage; byId("submit-plan-actions").hidden = !planSubmissionAvailable; const submitPlanButton = byId("submit-plan-button"); submitPlanButton.disabled = busy || !canSubmitPlan(snapshot, specSnapshot); submitPlanButton.textContent = retryPlanStage ? "Retry Plan send" : "Submit Plan as-is";
    ...
    const currentGrill = snapshot.grill?.basedOnDocumentRevision === snapshot.documentRevision; const toSpec = byId("to-spec-button"); toSpec.hidden = !["plan", "grill"].includes(activeStage) || !availability.spec; toSpec.disabled = busy || !availability.spec; toSpec.textContent = activeStage === "plan" && !currentGrill ? "Skip to Spec" : "Continue to To Spec";
  ...
  const addressGrillFeedback = () => { void runAction(async () => { const current = store.get(); if (!current.snapshot) return; const selected = selectedOpenGrillAnnotationIds(current.snapshot, current.selectedGrillAnnotationIds); if (!selected.length) return; await mutatePlan("/api/v1/revision-requests", "POST", revisionRequestPayload(selected, byId("grill-revision-instruction").value)); byId("grill-revision-instruction").value = ""; store.set({ selectedGrillAnnotationIds: [] }); }); };
  byId("address-grill-feedback").addEventListener("click", addressGrillFeedback); byId("review-plan-with-feedback-button").addEventListener("click", addressGrillFeedback);
  ...
  byId("spec-generate-button").addEventListener("click", () => { void runAction(async () => { const current = store.get(); const fresh = canGenerateFreshSpec(current.snapshot, current.specSnapshot); await mutateSpec(fresh ? "/api/v1/spec/fresh-generations" : "/api/v1/spec/generations", "POST", {}); if (fresh) store.set({ selectedSpecCommentIds: [] }); toast(fresh ? "Fresh Spec generation started from the current Plan and optional Adversarial Review." : "Spec generation started."); }); });
  ...
  const submitPlan = () => { void runAction(async () => { const current = store.get(); const snapshot = current.snapshot; if (!snapshot || !canSubmitPlan(snapshot, current.specSnapshot)) return; const retry = canRetryStaging(snapshot); if (!(await confirmDialog(retry ? "Retry sending this Plan?" : "Submit this Plan as-is?", retry ? "Pi will retry sending the exact accepted Plan." : "Pi will submit this exact durable Plan and end the review without requiring the optional later stages.", retry ? "Retry send" : "Submit Plan as-is"))) return; await mutatePlan("/api/v1/accept", "POST", { stateVersion: snapshot.stateVersion, documentRevision: snapshot.documentRevision, confirmed: true }); finishReview(); toast("The authoritative Plan was sent to the agent."); }); };
  byId("submit-plan-button").addEventListener("click", submitPlan);
  ...
  const stopSpec = (disposition) => { void runAction(async () => { if (!(await confirmDialog(disposition === "pause" ? "Pause Spec work?" : "Cancel this Spec?", disposition === "pause" ? "The independent Spec history remains saved." : "The Spec becomes terminal; the Plan and any Adversarial Review remain unchanged.", disposition === "pause" ? "Pause Spec" : "Cancel Spec"))) return; await mutateSpec("/api/v1/spec/cancel", "POST", { disposition }); }); };
