...
const privateState: PlanSession = {
  schemaVersion: 2, id: "session", stateVersion: 9, documentRevision: 2, status: "revising",
  source: { prompt: "SECRET PROMPT", cwd: "/private/cwd" }, generation: { mode: "careful" },
...
describe("browser protocol", () => {
  it("projects an explicit public allowlist and preserves hostile text as data", () => {
    ...
    for (const secret of ["/private/cwd", "PRIVATE INSTRUCTION", "PRIVATE TRAILING", "PRIVATE MODEL", "PRIVATE TEAM", "selectedAnnotationIds", "jobId", "token", "stack", "REPORT SECRET", "spawn_agent", "NONCE SECRET", "privateReport", "toolName", "teamName"]) expect(encoded).not.toContain(secret);
