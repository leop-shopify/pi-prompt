...
function session(document: PlanSession["document"] = null): PlanSession {
  return {
    schemaVersion: 2, id: "session", stateVersion: 2, documentRevision: document ? 1 : 0,
    status: document ? "revising" : "generating", source: { prompt: "Build it", cwd: "/repo" },
    generation: { mode: "normal" }, document, annotations: [],
...
describe("current-agent result validation", () => {
  it("accepts a strict initial result with actionable Implementation Tasks", () => {
    expect(validateGeneratorSubmission(initial, "initial", session())).toEqual({ ok: true, outcome: initial });
  ...
  it("accepts exact arbitrary revision Markdown without semantic grading or text mutation", () => {
    ...
    for (const markdown of values) {
      ...
      expect(validateGeneratorSubmission(revision, "revision", session(materialized as never))).toEqual({ ok: true, outcome: revision });
    }
  ...
  it("rejects wrong contracts with actionable paths and missing task fields", () => {
  ...
  it("validates strict Grill results against the exact revision", () => {
