...
import type { PlanOnlyCapturedSpecSource, PlanOnlySpecSourceReference, ReviewedCapturedSpecSource, ReviewedSpecSourceReference, SpecSession } from "../spec/types.js";

export const NOW = "2026-07-12T00:00:00.000Z";
export const MARKDOWN = "# Spec\n\nBuild 😀 safely.\n";
export function source(root = "/tmp/pi-prompt-plans", id = "plan-session"): ReviewedSpecSourceReference {
...
export function planOnlySource(root = "/tmp/pi-prompt-plans", id = "plan-session"): PlanOnlySpecSourceReference {
  const artifact = `${root}/${id}`; const annotations: unknown[] = [];
  return { planSessionId: id, planArtifactPath: artifact, planMarkdownPath: `${artifact}/plan.md`, annotationsPath: `${artifact}/annotations.json`, planDocumentRevision: 1, planStateVersion: 4, planMarkdownSha256: sha256Text("# Plan\n"), annotationsSha256: sha256Json(annotations) };
}
export function captured(root = "/tmp/pi-prompt-plans", id = "plan-session"): ReviewedCapturedSpecSource { return { reference: source(root, id), planMarkdown: "# Plan\n", annotations: [], decisionTree: { nodes: [] } }; }
export function planOnlyCaptured(root = "/tmp/pi-prompt-plans", id = "plan-session"): PlanOnlyCapturedSpecSource { return { reference: planOnlySource(root, id), planMarkdown: "# Plan\n", annotations: [] }; }
