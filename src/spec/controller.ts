...
import { hasAdversarialReview, sameSpecSource } from "./source.js";
...
export class SpecController {
  ...
  async accept(input: { readonly expectedStateVersion: number; readonly specRevision: number; readonly confirmed: boolean }): Promise<SpecResult> {
    ...
    let fresh; try { fresh = await this.#options.source.fresh(); } catch { return failure("source-unavailable", "Fresh Plan source could not be verified."); }
    return this.#enqueue(async () => {
      ...
      if (!sameSpecSource(captured.source, fresh.value.reference)) return failure("stale-spec-source", "Plan or optional Adversarial Review source changed before Spec acceptance.");
  ...
  async #start(operation: SpecOperation, expected: number, selected: readonly string[], instruction: string | undefined, rebase: boolean): Promise<SpecResult<SpecJobHandle>> {
    ...
    let fresh; try { fresh = await this.#options.source.fresh(); } catch { return failure("source-unavailable", "Fresh Plan source could not be verified."); } if (!fresh.ok) return fresh;
    const started = await this.#enqueue(async (): Promise<SpecResult<{ readonly state: SpecSession; readonly runtime: RuntimeJob }>> => {
      ...
      if (rebase && !sourceChanged) return failure("spec-source-current", "The Spec already uses the current Plan source.");
      if (!rebase && sourceChanged) return failure("stale-spec-source", "Plan or optional Adversarial Review source changed; generate a fresh Spec.");
...
export function formatAcceptedSpec(session: SpecSession): string {
  ...
  const review = hasAdversarialReview(source) ? [
  ...
  const dispatch = [
    ...
    "Use the source references below as context. The Adversarial Review is optional and its status is recorded explicitly. Follow the current session's normal permissions and instructions.",
